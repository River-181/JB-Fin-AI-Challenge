...
이 MVP는 전북은행, 광주은행, JB우리캐피탈을 중심으로 지역 소상공인과 중소사업자의 연체 조짐, 금리 부담, 정책금융 매칭, 보이스피싱 위험을 하나의 `Case`로 묶고, 각 Agent가 스킬을 장착해 판단, 행동, 검증을 수행하는 운영 콘솔입니다. UI는 HagentOS의 4-zone 운영 구조를 금융 도메인에 맞게 옮겨 대시보드, 알림함, 케이스, 승인 큐, AgentRun, 에이전트 팀, 스킬 레지스트리, Heartbeat, 목표, 처리 이력, 예산, 설정 항목을 제공합니다.
