...
Architecture images are rendered from 07_아키텍처/README.md mermaid sources.
"""
...
stat_specs = [
    ("8,545억 원", "2024년 보이스피싱 피해액 · 역대 최대", "전년 대비 약 2배 급증  |  경찰청 국가수사본부"),
    ("39,121건", "전세사기 피해자 누적 결정 (’26.5 기준)", "’23.6 특별법 시행 후 계속 증가  |  국토교통부"),
    ("43.7만 명", "취약 자영업자 (’25.2분기) · 대출 약 130조 원", "취약차주 연체율 11%대  |  한국은행 금융안정 상황"),
]
sw = (CW - 0.4) / 3
...
add_text(s, CX, CY - 0.02, CW, 0.24,
         [[("시스템 아키텍처  ", 11, INK, True),
           ("— UI · 케이스 런타임 · 에이전트 메시 · 스킬 저장소 · 승인 게이트 · 감사 기록 (GitHub 07_아키텍처 Mermaid 원본)", 9, MUTED, False)]])
...
add_text(s, bx + 0.14, sy + 0.09, bw - 0.28, 0.4, [("보조 유스케이스 — 소상공인 자금압박 케어", 9, INK, True)])
sub_steps = [
...
impact = [
    ...
    ("0건", "고위험 사기 케이스 외부 발송", "사기 차단 에이전트가 자동 차단 · 보안 보고만"),
]
iw = (CW - 0.6) / 4
